...
from acc.utils.number_theory import gcd, lcm, modinv, eea
from acc.utils.bitwise import ror, rol, xor
from acc.utils.stochastic import nCr, nPr
from acc.utils.prime import isprime, nextprime
from acc.utils.convert_base import (
    bin2dec,
    dec2bin,
    hex2dec,
    dec2hex,
...
def solve_equation(user_input, variables):
    ...
    try:
        ...
        sympy_vars.update(
            {var: symbols(var) for var in lhs.split() + rhs.split() if var.isalpha()}
        )
        ...
        equation = Eq(
            eval(
                lhs,
                {"__builtins__": None},
                {**SAFE_FUNCTIONS, **sympy_vars, **variables},
            ),
            eval(
                rhs,
                {"__builtins__": None},
    ...
    for func_name, func in SAFE_FUNCTIONS.items():
        if callable(func):
            ...
            functions_by_module[module].append(
                (func_name, signature, func.__doc__ or "No description available.")
            )
            all_items.append(
                (func_name, signature)
            )  # Add for global max width calculation
    ...
    for module, funcs in functions_by_module.items():
        ...
        for func_name, signature, doc in funcs:
            ...
            wrapped_description = textwrap.fill(
                doc, width=80, subsequent_indent=" " * max_width
            )
